...
<div class="container mt-5 py-5">
		<div class="addressInfo_div">
					<div class="addressInfo_button_div">
						<button class="address_btn address_btn_1" id="myInfo">내 정보</button>
						<button class="address_btn address_btn_2" id="myWrite">내가 쓴 글</button>

					</div>
					<div class="addressInfo_input_div_wrap">
						<div style="display: block" id="myPage">
						
							
						</div>
						<div style="display: block" id="myBoard">
						
							
							
						</div>
						<div style="display: block" id="myStore">	
							
						</div>
						<div style="display: block" id="mySell">
							
							
						</div>
							<div style="display: block" id="myBuy">
...
<script>
function getUserBoard(className){
	$.ajax({
		type:'GET',
		url:'/user/mylist',
		success:function(data){
			
			var str = '<br><br><br><h3>게시판 글</h3>';
			str+= '<table class="table table-hover table-condensed mt-5 mb-5">';
			str+= '<tr>';
			str+='<th>제목</th>';	
	        str+='<th>카테고리</th>';
	        str+='<th>작성자</th>';
			...
			$(data).each(function(){
				...
				str+='</td>';
				str+='<td>';
				var time= new Date(this.regdate);
				var date=time.toLocaleDateString(); 
				str+=date;
				str+='</td>';
				str+='<td>';
				var time2= new Date(this.updateDate);
				var date2=time2.toLocaleDateString(); 
				str+=date2;
				str+='</td>';
				str+='</tr>';
			});
...
function getUserStore(className){
	$.ajax({
		type:'GET',
		url:'/user/myStore',
		success:function(data){
			var str = '<h3>가게 홍보글</h3>';
			str+='<table class="table table-hover table-condensed mt-5 mb-5">';
			str+= '<tr>';
			str+='<th>가게이름</th>';	
	        str+='<th>카테고리</th>';
...
function getUserSell(className){
	$.ajax({
		type:'GET',
		url:'/user/mySell',
		success:function(data){
			var str = '<h3>나의 판매글</h3>';
			str+='<table class="table table-hover table-condensed mt-5 mb-5">';
			str+= '<tr>';
			str+='<th>제목</th>';	
	        str+='<th>작성자</th>';
			...
			$(data).each(function(){
				...
				str+='</td>';
				str+='<td>';
				str+=this.tradeWriter;
				str+='</td>';
				str+='<td>';
				var time= new Date(this.tradeRegdate);
				var date=time.toLocaleDateString(); 
				str+=date;
				str+='</td>';
				str+='<td>';
				var time2= new Date(this.tradeUpdatedate);
				var date2=time2.toLocaleDateString(); 
				str+=date2;
				str+='</td>';				
				str+='</tr>';
...
function getUserBuy(className){
	$.ajax({
		type:'GET',
		url:'/user/myBuy',
		success:function(data){
			var str = '<h3>나의 구매글</h3>';
			str+='<table class="table table-hover table-condensed mt-5 mb-5">';
			str+= '<tr>';
			str+='<th>제목</th>';	
	        str+='<th>작성자</th>';
			...
			$(data).each(function(){
				str+='<tr>';
				str+='<td>';
				str+=this.tradeTitle2;
				str+='</td>';
				str+='<td>';
				str+=this.tradeWriter2;
				str+='</td>';
				str+='<td>';
				var time= new Date(this.tradeRegdate2);
				var date=time.toLocaleDateString(); 
				str+=date;
				str+='</td>';
				str+='<td>';
				var time2= new Date(this.tradeUpdatedate2);
				var date2=time2.toLocaleDateString(); 
				str+=date2;
				str+='</td>';				
				str+='</tr>';
